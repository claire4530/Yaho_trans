...
//     );            
// }
// app/[locale]/services/project/one-stop/page.tsx
import { createClient } from '@/src/utils/supabase/server';
import OneStopClient from '@/src/components/ui/OneStopClient';
import { notFound } from 'next/navigation';

export default async function OneStopPage() {
  const supabase = await createClient();

  // 專門抓取 slug = 'one-stop' 的資料
  ...
  // 雖然是寫死路徑，但還是防呆一下
  if (!service) {
    // 如果資料庫不小心刪了這筆，還是可以顯示頁面，只是沒有圖片資料
    console.error("One-stop data not found in DB");
  }

  return <OneStopClient service={service || {}} />;
}